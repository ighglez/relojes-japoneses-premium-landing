...
function RegisterContent() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (!formData.name.trim()) return toast.error("Por favor, ingresa tu nombre completo");
    if (!formData.email.trim()) return toast.error("Por favor, ingresa tu email");
        ...
        <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.5 }} className="w-full max-w-md">
          <div className="bg-white rounded-xl shadow-lg p-8">
            <div className="text-center mb-8">
              <h1 className="font-heading text-3xl font-medium text-graphite mb-2">Crear cuenta</h1>
              <p className="text-graphite/70">Únete y accede al sistema de referidos</p>
            </div>
            ...
            <form onSubmit={handleSubmit} className="space-y-5">
              <div>
                <Label htmlFor="name" className="text-graphite font-medium">Nombre completo *</Label>
                <Input id="name" type="text" required placeholder="Juan Pérez"
                  value={formData.name} onChange={(e) => setFormData({ ...formData, name: e.target.value })}
                  className="mt-1.5 h-11" aria-required="true" disabled={loading || googleLoading} />
              </div>
              ...
              <div>
                ...
                <div className="relative mt-1.5">
                  <Input id="password" type={showPassword ? "text" : "password"} required minLength={8}
                    ...
                    className="pr-10 h-11" aria-required="true" disabled={loading || googleLoading} />
                ...
                {formData.password && (
                  <div className="mt-2 flex items-center gap-2 text-xs">
                    {passwordValidations.length ? <CheckCircle2 className="h-3.5 w-3.5 text-green-600" /> : <XCircle className="h-3.5 w-3.5 text-red-500" />}
                    <span className={passwordValidations.length ? "text-green-700" : "text-red-600"}>Mínimo 8 caracteres</span>
                  </div>
                )}
              </div>

              <div>
                <Label htmlFor="confirmPassword" className="text-graphite font-medium">Confirmar contraseña *</Label>
                <div className="relative mt-1.5">
                ...
                {formData.confirmPassword && (
                  <div className="mt-2 flex items-center gap-2 text-xs">
                    {passwordValidations.match ? <CheckCircle2 className="h-3.5 w-3.5 text-green-600" /> : <XCircle className="h-3.5 w-3.5 text-red-500" />}
                    <span className={passwordValidations.match ? "text-green-700" : "text-red-600"}>Las contraseñas coinciden</span>
                  </div>
                )}
              </div>
